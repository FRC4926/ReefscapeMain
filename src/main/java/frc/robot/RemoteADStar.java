...
public class RemoteADStar implements Pathfinder {
    ...
    private final AtomicReference<List<PathPoint>> currentPath = new AtomicReference<>(new ArrayList<>());
    private final AtomicBoolean newPathAvailable = new AtomicBoolean(false);

    /**
     * Create a RemoteADStar object. This will use NT4 to communicate with the
     * coprocessor.
     */
    public RemoteADStar() {
        ...
        dynamicObsPub = nt.getDoubleArrayTopic("/PPLibCoprocessor/RemoteADStar/dynamicObstacles").publish();

        pathPointsSub = nt.getDoubleArrayTopic("/PPLibCoprocessor/RemoteADStar/pathPoints")
                .subscribe(
